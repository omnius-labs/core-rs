...
mod tests {
    ...
    fn temp_dir(name: &str) -> PathBuf {
        let unique = SystemTime::now().duration_since(UNIX_EPOCH).expect("time went backwards").as_nanos();
        let dir = std::env::temp_dir().join(format!("omnius-core-rocketpack-compiler-{name}-{unique}"));
        fs::create_dir_all(&dir).expect("create temp dir");
        dir
    }

    fn parse_test_source(source: &str) -> File {
        parse_source("virtual.rpf", source).expect("parse source")
    }

    fn find_struct<'a>(file: &'a File, name: &str) -> &'a Struct {
        file.items
            .iter()
            .find_map(|item| match item {
                Item::Struct(item) if item.name.value == name => Some(item),
                _ => None,
            })
            .expect("struct not found")
    }

    fn string_mapping(entries: &[(&str, &str)]) -> Mapping {
    ...
    fn rust_generator_config(targets: Vec<GeneratorTargetConfig>) -> GeneratorConfig {
        GeneratorConfig {
            id: "rust".to_string(),
    ...
    #[test]
    fn glob_matches_test() {
        assert!(glob_matches("**/*.rpf", "example/first.rpf"));
        assert!(glob_matches("example/*.rpf", "example/first.rpf"));
        assert!(glob_matches("example/**/first.rpf", "example/nested/first.rpf"));
        assert!(!glob_matches("*.rpf", "example/first.rpf"));
        assert!(!glob_matches("**/*.test.rpf", "example/first.rpf"));
    }

    #[test]
    fn discover_source_files_filters_by_include_and_exclude() {
        let root_dir = temp_dir("discover");
        let base_dir = root_dir.join("rpfs");
        fs::create_dir_all(base_dir.join("example/nested")).expect("create source tree");
        fs::write(base_dir.join("example/first.rpf"), "version 1;").expect("write first.rpf");
        fs::write(base_dir.join("example/second.test.rpf"), "version 1;").expect("write second.test.rpf");
        fs::write(base_dir.join("example/nested/third.rpf"), "version 1;").expect("write third.rpf");

        let sources = vec![SourceConfig {
            base_dir: "rpfs".to_string(),
            includes: vec!["**/*.rpf".to_string()],
            excludes: vec!["**/*.test.rpf".to_string()],
        }];

        let discovered = discover_source_files(&root_dir, &sources).expect("discover source files");
        let relative_paths = discovered.iter().map(|source| normalize_path(&source.relative_path)).collect::<Vec<_>>();

        assert_eq!(relative_paths, vec!["example/first.rpf", "example/nested/third.rpf"]);

        fs::remove_dir_all(root_dir).expect("cleanup temp dir");
    }

    #[test]
    fn resolve_output_path_uses_first_matching_target() {
        let root_dir = PathBuf::from("/tmp/rocketpack-compiler-test");
        let conf = rust_generator_config(vec![
            GeneratorTargetConfig {
                pattern: "example/first.rpf".to_string(),
                options: Some(string_mapping(&[("dir", "rust/gen/src/example/first")])),
            },
            GeneratorTargetConfig {
                pattern: "example/*.rpf".to_string(),
                options: Some(string_mapping(&[("dir", "rust/gen/src/example/second")])),
            },
        ]);
        let source = DiscoveredSource {
            base_dir: PathBuf::from("rpfs"),
            absolute_path: root_dir.join("rpfs/example/first.rpf"),
            relative_path: PathBuf::from("example/first.rpf"),
        };

        let output_path = resolve_output_path(&root_dir, &conf, &source).expect("resolve output path");

        assert_eq!(output_path, Some(root_dir.join("rust/gen/src/example/first/first.rs")));
    }

    #[test]
    fn write_generated_files_writes_only_matched_targets() {
        let root_dir = temp_dir("write-generated-files");
        let conf = rust_generator_config(vec![GeneratorTargetConfig {
            pattern: "example/first.rpf".to_string(),
            options: Some(string_mapping(&[("dir", "rust/gen/src/example/first")])),
        }]);
        let generated_files = vec![
            GeneratedRustFile {
                ...
                contents: "pub const MATCHED: u32 = 1;\n".to_string(),
            },
            GeneratedRustFile {
                source: DiscoveredSource {
                    base_dir: PathBuf::from("rpfs"),
                    absolute_path: root_dir.join("rpfs/example.rpf"),
                    relative_path: PathBuf::from("example.rpf"),
                },
                contents: "pub const SKIPPED: u32 = 2;\n".to_string(),
            },
        ];

        let written_count = write_generated_files(&root_dir, &conf, &generated_files).expect("write generated files");
        let matched_path = root_dir.join("rust/gen/src/example/first/first.rs");
        let skipped_path = root_dir.join("rust/gen/src/example/example.rs");

        assert_eq!(written_count, 1);
        assert_eq!(fs::read_to_string(&matched_path).expect("read matched output"), "pub const MATCHED: u32 = 1;\n");
    ...
    #[test]
    fn render_example_file_declarations_test() {
        let source = fs::read_to_string("./data/rpfs/example.rpf").expect("read example.rpf");
        let file = parse_source("example.rpf", &source).expect("parse example.rpf");
        let parsed_source = ParsedSource {
            source: DiscoveredSource {
                base_dir: PathBuf::from("rpfs"),
                absolute_path: PathBuf::from("example.rpf"),
                relative_path: PathBuf::from("example.rpf"),
            },
            file,
        };

        let rendered = render_rust_file(&parsed_source).expect("render rust file");

        assert!(rendered.contains("pub mod omnius {"));
        assert!(rendered.contains("pub mod demo {"));
        assert!(rendered.contains("pub mod v1 {"));
        assert!(rendered.contains("pub struct PrimitiveShowcase1 {"));
        assert!(rendered.contains("pub struct PrimitiveShowcase2 {"));
        assert!(rendered.contains("pub enum PrimitiveShowcase3 {"));
        assert!(rendered.contains("Second {"));
        assert!(rendered.contains("payload: ByteBuffer,"));
        assert!(rendered.contains("status: Status,"));
        assert!(rendered.contains("pub type ByteList = Vec<Vec<u8>>;"));
        assert!(rendered.contains("pub const MAX_SAMPLE_SIZE: u32 = 1048576;"));
    }

    #[test]
    fn render_example_file_struct_codec_test() {
        let source = fs::read_to_string("./data/rpfs/example.rpf").expect("read example.rpf");
        let file = parse_source("example.rpf", &source).expect("parse example.rpf");
        let parsed_source = ParsedSource {
            source: DiscoveredSource {
        ...
        assert!(rendered.contains("impl omnius_core_rocketpack::RocketPackStruct for PrimitiveShowcase1 {"));
        assert!(rendered.contains("encoder.write_map(22)?;"));
        assert!(rendered.contains("encoder.write_bytes((&value.bytes_field).as_slice())?;"));
        assert!(rendered.contains("encoder.write_array((&value.vec_field_2).len())?;"));
        assert!(rendered.contains("encoder.write_map((&value.map_field_1).len())?;"));
        assert!(rendered.contains("let count = decoder.read_map()?;"));
        assert!(rendered.contains("_ => decoder.skip_field()?,"));
        assert!(rendered.contains("array length mismatch: slice_field"));
        assert!(rendered.contains("decoder.read_struct::<SimpleMessage>()?"));
        assert!(rendered.contains("u128 encode is not supported"));
        assert!(rendered.contains("i128 decode is not supported"));
        assert!(rendered.contains("impl omnius_core_rocketpack::RocketPackStruct for PrimitiveShowcase2 {"));
        assert!(rendered.contains("let mut count = 0;"));
        assert!(rendered.contains("if value.bool_field.is_some() {"));
        assert!(rendered.contains("if let Some(struct_field) = &value.struct_field {"));
    }

    #[test]
    fn render_example_file_enum_codec_test() {
        let source = fs::read_to_string("./data/rpfs/example.rpf").expect("read example.rpf");
        let file = parse_source("example.rpf", &source).expect("parse example.rpf");
        let parsed_source = ParsedSource {
            source: DiscoveredSource {
        ...
        assert!(rendered.contains("impl omnius_core_rocketpack::RocketPackStruct for PrimitiveShowcase3 {"));
        assert!(rendered.contains("Self::First => {"));
        assert!(rendered.contains("encoder.write_u64(1)?;"));
        assert!(rendered.contains("encoder.write_map(0)?;"));
        assert!(rendered.contains("Self::Second { entity, payload } => {"));
        assert!(rendered.contains("encoder.write_u64(2)?;"));
        assert!(rendered.contains("encoder.write_map(2)?;"));
        assert!(rendered.contains("encoder.write_u64(0)?;"));
        assert!(rendered.contains("Self::Third { entity, status, retries, struct_field } => {"));
        assert!(rendered.contains("let mut count = 3;"));
        assert!(rendered.contains("if struct_field.is_some() {"));
        assert!(rendered.contains("encoder.write_u64(4)?;"));
        assert!(rendered.contains("encoder.write_u64(5)?;"));
        assert!(rendered.contains("let mut result: Option<Self> = None;"));
        assert!(rendered.contains("let __inner_count_"));
        assert!(rendered.contains("= decoder.read_map()?;"));
        assert!(rendered.contains("result = Some(Self::Second { entity: entity.ok_or"));
        assert!(rendered.contains("retries: retries.unwrap_or(0),"));
        assert!(rendered.contains("_ => decoder.skip_field()?,"));
        assert!(rendered.contains("missing enum variant"));
    }

    #[test]
    fn render_struct_codec_uses_default_for_missing_field() {
        let file = parse_test_source(
            r#"
version 1;

struct RetryMessage {
  @1 retries: u32 = 0;
  @2 label: Option<string>;
}
"#,
        );
        let parsed_source = ParsedSource {
            source: DiscoveredSource {
                base_dir: PathBuf::from("rpfs"),
                absolute_path: PathBuf::from("virtual.rpf"),
                relative_path: PathBuf::from("virtual.rpf"),
            },
            file,
        };
        ...
        assert!(rendered.contains("impl omnius_core_rocketpack::RocketPackStruct for RetryMessage {"));
        assert!(rendered.contains("let mut count = 1;"));
        assert!(rendered.contains("if value.label.is_some() {"));
        assert!(rendered.contains("retries: retries.unwrap_or(0),"));
        assert!(rendered.contains("label: label,"));
        assert!(!rendered.contains("missing field: retries"));
    }

    #[test]
    fn resolve_type_expands_alias_and_imports() {
        let file = parse_test_source(
            r#"
version 1;
package demo::sample;
use external::pkg::Thing as ImportedThing;

type ByteBuffer = Vec<bytes>;

struct Message {
  @1 payload: ByteBuffer;
  @2 imported: ImportedThing;
  @3 direct: external::pkg::OtherThing;
}
"#,
        );
        let index = build_schema_index(&file);
        let message = find_struct(&file, "Message");

        let payload = resolve_type(&index, &message.fields[0].ty.value).expect("resolve payload");
        let imported = resolve_type(&index, &message.fields[1].ty.value).expect("resolve imported");
        let direct = resolve_type(&index, &message.fields[2].ty.value).expect("resolve direct");

        assert_eq!(payload, ResolvedType::Vec(Box::new(ResolvedType::Builtin(BuiltinType::Bytes))));
        assert_eq!(
            imported,
            ResolvedType::Named(NamedType {
                rust_path: vec!["external".into(), "pkg".into(), "Thing".into()],
                kind: NamedTypeKind::External,
            })
        );
        assert_eq!(
            direct,
            ResolvedType::Named(NamedType {
                rust_path: vec!["external".into(), "pkg".into(), "OtherThing".into()],
                kind: NamedTypeKind::External,
            })
        );
    }
    ...
    #[test]
    fn render_declarations_preserve_alias_and_use_names() {
        let file = parse_test_source(
            r#"
version 1;
package demo::sample;
...
"#,
        ...
        assert!(rendered.contains("use external::pkg::Thing as ImportedThing;"));
        assert!(rendered.contains("pub type ByteBuffer = Vec<Vec<u8>>;"));
        assert!(rendered.contains("pub payload: ByteBuffer,"));
        assert!(rendered.contains("pub imported: ImportedThing,"));
    }
}
