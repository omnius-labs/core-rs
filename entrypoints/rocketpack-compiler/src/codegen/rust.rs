...
use crate::{
    ...
    semantic::{BuiltinType, ResolvedType, SchemaSymbol, SemanticGraph, builtin_type, path_segments},
};
    ...
    for (field, resolved) in fields {
        ...
        match resolved {
            ResolvedType::Option(inner) => {
                ...
                write_validate_value(out, index, inner, &field_ident, depth + 2, &context)?;
                writeln!(out, "{}}}", indent(depth + 1)).ok();
            }
            _ => write_validate_value(out, index, resolved, &format!("&value.{field_ident}"), depth + 1, &context)?,
        }
    }
...
fn write_validate_value(out: &mut String, index: &RustFileContext, resolved: &ResolvedType, expr: &str, depth: usize, context: &str) -> Result<(), CodegenError> {
    match resolved {
        ResolvedType::Constrained(inner, constraint) => {
            ...
            .ok();
            write_validate_value(out, index, inner, expr, depth, context)?;
        }
        ResolvedType::Option(inner) => write_validate_value(out, index, inner, expr, depth, context)?,
        ResolvedType::Vec(inner) | ResolvedType::Array(inner, _) => {
            if has_length_constraints(inner) {
                ...
                write_validate_value(out, index, inner, "item", depth + 1, &format!("{context}[]"))?;
                writeln!(out, "{}}}", indent(depth)).ok();
            }
        }
        ResolvedType::Map(key, value) => {
            ...
            if validate_key || validate_value {
                ...
                if validate_key {
                    write_validate_value(out, index, key, key_name, depth + 1, &format!("{context}.key"))?;
                }
                if validate_value {
                    write_validate_value(out, index, value, value_name, depth + 1, &format!("{context}.value"))?;
                }
                writeln!(out, "{}}}", indent(depth)).ok();
            }
        }
        ResolvedType::Named(key) => {
            writeln!(
                out,
                "{}<{} as omnius_core_rocketpack::RocketPackStruct>::validate({})?;",
                indent(depth),
                render_named_type(index, key),
                expr
            )
            .ok();
        }
        ResolvedType::Builtin(BuiltinType::Timestamp64 | BuiltinType::Timestamp96) => {
            writeln!(
                ...
                "{}<{} as omnius_core_rocketpack::RocketPackStruct>::validate({})?;",
                indent(depth),
                render_resolved_type(index, resolved),
                expr
            )
            .ok();
        }
...
fn write_struct_unpack_fn(out: &mut String, index: &RustFileContext, item: &Struct, fields: &[(&Field, ResolvedType)], depth: usize) -> Result<(), CodegenError> {
    ...
    for (field, resolved) in fields {
        ...
        let context = format!("{}.{}", item.name.value, field.name.value);
        let value_expr = write_decode_value(out, index, resolved, "decoder", depth + 4, &context, &mut temp_counter)?;
...
fn render_field_storage_type(index: &RustFileContext, field: &Field, resolved: &ResolvedType) -> String {
    match resolved {
        ResolvedType::Option(_) => render_resolved_type(index, resolved),
...
fn write_decode_value(
    ...
    match resolved {
        ResolvedType::Builtin(builtin) => Ok(match builtin {
            ...
            BuiltinType::Timestamp64 | BuiltinType::Timestamp96 => {
                format!("{decoder_ident}.read_struct::<{}>()?", render_resolved_type(index, resolved))
            }
        ...
        ResolvedType::Named(key) => Ok(format!("{decoder_ident}.read_struct::<{}>()?", render_named_type(index, key))),
        ResolvedType::Option(inner) => {
            ...
            let inner_expr = write_decode_value(out, index, inner, decoder_ident, depth + 1, context_name, temp_counter)?;
        ...
        ResolvedType::Constrained(inner, constraint) => match inner.as_ref() {
            ...
            ResolvedType::Vec(inner) => {
                ...
                .ok();
                writeln!(out, "{}let mut {}: Vec<{}> = Vec::new();", indent(depth), value_name, render_resolved_type(index, inner)).ok();
                writeln!(out, "{}for _ in 0..{} {{", indent(depth), count_name).ok();
                let inner_expr = write_decode_value(out, index, inner, decoder_ident, depth + 1, &format!("{context_name}[]"), temp_counter)?;
            ...
            ResolvedType::Map(key, value) => {
                ...
                writeln!(
                    ...
                    indent(depth),
                    map_name,
                    render_resolved_type(index, key),
                    render_resolved_type(index, value)
                )
                .ok();
                writeln!(out, "{}for _ in 0..{} {{", indent(depth), count_name).ok();
                let key_expr = write_decode_value(out, index, key, decoder_ident, depth + 1, &format!("{context_name}.key"), temp_counter)?;
                let key_name = next_temp_name(temp_counter, "key");
                writeln!(out, "{}let {} = {};", indent(depth + 1), key_name, key_expr).ok();
                let value_expr = write_decode_value(out, index, value, decoder_ident, depth + 1, &format!("{context_name}.value"), temp_counter)?;
        ...
        ResolvedType::Array(inner, len) => {
            ...
            .ok();
            writeln!(out, "{}}}", indent(depth)).ok();
            writeln!(out, "{}let mut {}: Vec<{}> = Vec::new();", indent(depth), values_name, render_resolved_type(index, inner)).ok();
            writeln!(out, "{}for _ in 0..{} {{", indent(depth), count_name).ok();
            let inner_expr = write_decode_value(out, index, inner, decoder_ident, depth + 1, &format!("{context_name}[]"), temp_counter)?;
...
fn render_resolved_type(index: &RustFileContext, resolved: &ResolvedType) -> String {
    match resolved {
        ResolvedType::Builtin(builtin) => match builtin {
        ...
        ResolvedType::Named(key) => render_named_type(index, key),
        ResolvedType::Option(inner) => format!("Option<{}>", render_resolved_type(index, inner)),
        ResolvedType::Vec(inner) => format!("Vec<{}>", render_resolved_type(index, inner)),
        ResolvedType::Map(key, value) => format!("std::collections::BTreeMap<{}, {}>", render_resolved_type(index, key), render_resolved_type(index, value)),
        ResolvedType::Array(inner, len) => format!("[{}; {}]", render_resolved_type(index, inner), len),
        ResolvedType::Constrained(inner, _) => render_resolved_type(index, inner),
    }
}

fn render_named_type(index: &RustFileContext, key: &str) -> String {
    render_symbol_path(index, index.graph.symbol(key))
            ...
            for ((_, _), (position, resolved)) in fields.iter().zip(resolved_fields.iter()) {
                ...
                match resolved {
                    ResolvedType::Option(inner) => {
                        ...
                        write_validate_value(out, index, inner, &binding, depth + 2, &context)?;
                        writeln!(out, "{}}}", indent(depth + 1)).ok();
                    }
                    _ => write_validate_value(out, index, resolved, &binding, depth + 1, &context)?,
                }
            }
            writeln!(out, "{}}},", indent(depth)).ok();
        }
    ...
    match &variant.kind {
        ...
        VariantKind::Tuple(fields) => {
            ...
            for ((_, (tuple_index, resolved)), binding_name) in fields.iter().zip(resolved_fields.iter()).zip(tuple_bindings.iter()) {
                ...
                let value_expr = write_decode_value(out, index, resolved, "decoder", depth + 4, &context, temp_counter)?;
...
fn resolve_type(index: &RustFileContext, ty: &Type) -> Result<ResolvedType, CodegenError> {
    index.graph.resolve_type(index.file_index, ty)
}
...
fn render_semantic_builtin(builtin: &BuiltinType) -> &'static str {
    match builtin {
        ...
        BuiltinType::Timestamp64 => "omnius_core_rocketpack::primitive::Timestamp64",
        BuiltinType::Timestamp96 => "omnius_core_rocketpack::primitive::Timestamp96",
    }
}
