...
impl std::fmt::Display for ErrorKind {
    fn fmt(&self, fmt: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            ErrorKind::YamuxError => write!(fmt, "yamux error"),
            ErrorKind::AcceptDisabled => write!(fmt, "accept disabled"),
            ErrorKind::ConnectionClosed => write!(fmt, "connection closed"),
        }
    }
}
...
impl From<yamux::ConnectionError> for Error {
    fn from(e: yamux::ConnectionError) -> Self {
        Error::from_error(e, ErrorKind::YamuxError).with_message("yamux error")
    }
}
