...
pub struct SesSenderMock {
    pub send_mail_simple_text_inputs: RefCell<Vec<SendMailSimpleTextInput>>,
}
...
impl SesSender for SesSenderMock {
    async fn send_mail_simple_text(&self, to_address: &str, from_address: &str, subject: &str, text_body: &str) -> anyhow::Result<()> {
        self.send_mail_simple_text_inputs.borrow_mut().push(SendMailSimpleTextInput {
...
impl SesSenderMock {
    ...
    pub fn new() -> Self {
        Self {
            send_mail_simple_text_inputs: RefCell::new(vec![]),
        }
    }
}
